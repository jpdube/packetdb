...
impl Tokenizer {
    ...
    // fn is_integer(&self, line: &String) -> Option<u32> {
    //     let start = self.index;
    //     let mut index = 0;

    //     index = start;

    //     while &String::from(line[index] as &str.is_numeric() && index <= line.len() {
    //         index += 1;
    //     }

    //     if index > start {
    //         return Some(999 as u32);
    //     }
...
mod tests {
    ...
    fn single_index_token() {
        ...
        println!("Index token len: {:?}", token_list);
    ...
    fn range_index_token() {
        ...
        assert!(token_list[0].token == Keyword::IndexStart);
        assert!(token_list[1].token == Keyword::Integer);
        assert!(token_list[2].token == Keyword::IndexRange);
