...
impl DbEngine {
    ...
    pub fn run(&mut self, query: &str) -> Result<&QueryResult, String> {
        ...
        if let Ok(expr) = parse.parse_select(query) {
            ...
            let mut index_manager = IndexManager::default();
            ...
            if let Some(interval) = &expr.interval {
                ...
                println!("Master index: {:?}", master_index);
                self.exec_plan.stop();
                info!("Using master index, {} found", master_index.len());
                for mp in master_index {
                    pkt_list.push(mp.file_ptr);
                }
            } else {
                self.exec_plan
                    .start("Packet index without master index search");
                info!("Scanning indexes");
                if let Ok(mut plist) = self.get_index_files() {
                    pkt_list.append(&mut plist);
                }
            ...
            for file_id in pkt_list {
                ...
                let ptr = index_manager.search_index(&expr, file_id);
    ...
    //         if let Ok(pkt_list) = self.get_index_files() {
    //             for file_id in pkt_list {
    //                 file_count += 1;
    //                 let interpreter = Interpreter::new(expr.clone());
