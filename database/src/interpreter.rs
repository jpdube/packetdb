...
impl Display for Object {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        match self {
            ...
            Object::LabelByte(field, start, end) => {
                write!(f, "Label byte: {},{},{}", field, start, end)
            }
...
impl Object {
    pub fn debug_type(&self) -> String {
        match self {
            ...
            Object::LabelByte(_, _, _) => "LabelByte",
            Object::Null => "Null",
        }
        .to_string()
    }
...
impl Interpreter {
    ...
    fn eval_expression(&self, expression: &Expression, pkt: &Packet) -> Result<Object, EvalError> {
        match expression {
            ...
            Expression::LabelByte(value, start, length) => Ok(Object::ByteArray(
                pkt.get_field_byte(*value, *start, *length) as Vec<u8>,
            )),
            Expression::IPv4(addr, mask) => Ok(Object::IPv4(*addr, *mask)),
            Expression::ByteArray(byte_data) => Ok(Object::ByteArray(byte_data.to_vec())),
    ...
    fn eval_infix_expression(
        ...
        match (&left, &right) {
            ...
            (Object::ByteArray(src_array), Object::ByteArray(cmp_array)) => {
                self.eval_array_infix_expression(operator, src_array.to_vec(), cmp_array.to_vec())
            }
    ...
    fn eval_array_infix_expression(
        ...
        match operator {
            Operator::Equal => {
                let matching = i.iter().zip(&j).filter(|&(i, j)| i == j).count();
                // println!("Byte comp: {}:{}:{}", matching, i.len(), j.len());
                Ok(Object::get_bool(matching == i.len() && matching == j.len()))
            }
            _ => Err(EvalError::UnknownOperator(
                format!("{}", operator),
                "eval_integer_infix_expression".to_string(),
