...
impl Tcp {
    ...
    pub fn seq_no(&self) -> u32 {
        BigEndian::read_u32(&self.raw_packet[self.start + 4..self.start + 8])
    }

    pub fn ack_no(&self) -> u32 {
        BigEndian::read_u32(&self.raw_packet[self.start + 8..self.start + 12])
    }
    ...
    pub fn hdr_len(&self) -> u8 {
        let hdr_field = self.raw_packet[self.start + 12];
        hdr_field >> 4
    }
    ...
    pub fn dport(&self) -> u16 {
        BigEndian::read_u16(&self.raw_packet[self.start + 2..self.start + 4])
    }

    pub fn sport(&self) -> u16 {
        BigEndian::read_u16(&self.raw_packet[self.start + 0..self.start + 2])
    }
    ...
    fn decode_options(&mut self) {
        if self.hdr_len() > 5 {
            ...
            let options = self.raw_packet[self.start + start_pos..self.start + end_pos].to_vec();
            let mut ptr: usize = 0;
...
impl Layer for Tcp {
    ...
    fn get_field_bytes(&self, _field_name: u32, start: usize, length: usize) -> Option<Vec<u8>> {
        ...
        let slice = self.raw_packet[self.start + start..self.start + start + length].to_vec();
        println!("TCP splice: {:?}", slice);
        Some(slice)
    }
