...
impl IpFrame {
    ...
    pub fn header_len(&self) -> u8 {
        let hdr_len = self.ip_packet[self.start + 0];

        hdr_len & 0x0f
    }

    pub fn tos(&self) -> u8 {
        self.ip_packet[self.start + 1]
    }

    pub fn ttl(&self) -> u8 {
        self.ip_packet[self.start + 8]
    }

    pub fn src(&self) -> u32 {
        BigEndian::read_u32(&self.ip_packet[self.start + 12..self.start + 16])
    }

    pub fn dst(&self) -> u32 {
        BigEndian::read_u32(&self.ip_packet[self.start + 16..self.start + 20])
    }

    pub fn proto(&self) -> u8 {
        self.ip_packet[self.start + 9]
    }
}
...
impl Layer for IpFrame {
    ...
    fn get_field_bytes(&self, _field_name: u32, start: usize, length: usize) -> Option<Vec<u8>> {
        if start + length > self.ip_packet.len() {
            return None;
        }

        let slice = self.ip_packet[self.start + start..self.start + start + length].to_vec();
        println!("IP splice: {:?}", slice);
        Some(slice)
    }
}
