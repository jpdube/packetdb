...
impl Packet {
    ...
    pub fn set_packet(&mut self, packet: Vec<u8>, header: [u8; 16], file_id: u32, pkt_ptr: u32) {
        ...
        self.raw_packet = Rc::new(packet.clone());
        ...
        ether.set_packet(0, vo, Rc::clone(&self.raw_packet));
        ...
        if ether.ethertype() == ETHER_IPV4_PROTO {
            ...
            ip_packet.set_packet(vo, vo + ip_header_len, Rc::clone(&self.raw_packet));
    ...
    pub fn get_field_byte(&self, _field: u32, start: usize, length: usize) -> Vec<u8> {
        // println!("Byte array: {}, {}, {}", _field, start, length);
        match _field {
            fields::ETH_BASE => {
                if self.has_ethernet() {
                    let eth = self.eth_packet.as_ref().unwrap();
                    if let Some(result) = eth.get_field_bytes(0, start, length) {
                        return result;
                    } else {
                        Vec::new()
                    }
            ...
            fields::IPV4_BASE => {
                if self.has_ipv4() {
                    let ipv4 = self.ip_packet.as_ref().unwrap();
                    if let Some(result) = ipv4.get_field_bytes(0, start, length) {
                        return result;
                    } else {
                        Vec::new()
                    }
            ...
            fields::TCP_BASE => {
                if self.has_tcp() {
                    let tcp = self.tcp_packet.as_ref().unwrap();
                    if let Some(result) = tcp.get_field_bytes(0, start, length) {
                        return result;
                    } else {
                        Vec::new()
                    }
